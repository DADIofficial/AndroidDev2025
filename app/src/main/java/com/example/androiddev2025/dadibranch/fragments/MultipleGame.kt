...
//import com.example.androiddev2025.ui.gamelist.LobbyAdapter
//import com.example.androiddev2025.ui.gamelist.LobbyUiModel
...
class MultipleGame : Fragment(R.layout.fragment_multiple_game) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        loadLobbies(recycler)

        view.findViewById<MaterialButton>(R.id.create_lobby_button).setOnClickListener {
            findNavController()
                .navigate(R.id.createLobbyFragment)
        }
    }

    private fun loadLobbies(recycler: RecyclerView) {
        CoroutineScope(Dispatchers.Main).launch {
            ...
            val data = lobbies.map { lobby ->
                ...
                LobbyData(
                    id = lobby.id,
                    name = lobby.name,
                    currentPlayers = lobby.playersIds.split(",").size,
            ...
            recycler.adapter = LobbyAdapter(data) {
                val bundle = Bundle().apply {
                    putInt("lobbyId", it.id)
                }
                findNavController()
                    .navigate(R.id.lobbyDetailsFragment, bundle)
            }
        }
    }
}
